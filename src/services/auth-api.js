...
const useAuthApi = () => {
  ...
  const handleLogin = async (
    ...
    try {
      ...
      localStorage.setItem('email', response.data.user.email); // Store the user's name
  ...
  const handleForgotPassword = async (email, setIsLoading, setSendLink) => {
    setIsLoading(true);
    try {
  ...
  const handleVerifyEmail = async (userId, navigate) => {
    try {
  ...
  // OTP
  const handleSendOtp = async (setSendingOtp, setSendOtp) => {
    setSendingOtp(true);
    try {
  ...
  // eslint-disable-next-line
  const handleVerifyToken = async (token, setIsLoggedIn) => {
    try {
